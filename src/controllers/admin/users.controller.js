...
const ajaxDatatablesUsers = async (req, res) => {
	...
	const data = dataUsers.reduce((arrDataUsers, currentDataUser) => {
		arrDataUsers.push([
			currentDataUser.id,
			`<div class="d-flex align-items-center">
					<div class="recent-product-img">
					<img src="${currentDataUser.avatar}" alt="">
					</div>
					<div class="ms-2">
					<h6 class="mb-1 font-14">${currentDataUser.name}</h6>
					</div>
				</div>`,
			currentDataUser.email,
			currentDataUser.level === 0
				? `<span class="badge text-white bg-gradient-lush text-uppercase px-3">Admin</span>`
				: currentDataUser.level === 1
			...
			``,
			currentDataUser.createdAt.toISOString().substring(0, 10),
			currentDataUser.updatedAt.toISOString().substring(0, 10),
			deleted
				? `<div class="d-flex order-actions">
						<a href="javascript:;" class="ms-1 btn-restore" onclick="restoreUser('${currentDataUser._id.toString()}')"><i class="bx bx-undo"></i></a>
						<a href="javascript:;" class="text-danger ms-1" onclick="fillDataToDeletePermanentlyForm('${
							currentDataUser.name
						}','${currentDataUser._id.toString()}')" data-bs-toggle="modal" data-bs-target="#deleteModal"><i class="bx bxs-trash"></i></a>
					</div>`
				: `<div class="d-flex order-actions">
						<a href="javascript:;" class="text-white"><i class="bx bx-detail"></i></a>
						<a href="javascript:;" class="text-warning ms-1" onclick="fillDataToEditForm('${currentDataUser._id.toString()}')" data-bs-toggle="modal" data-bs-target="#editModal"><i class="bx bxs-edit"></i></a>
